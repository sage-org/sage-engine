...
ExecutionResults = Tuple[List[Dict[str, str]], List[List[float]], bool, Optional[str]]


async def executor(
    pipeline: PreemptableIterator, context: Dict[str, Any] = {}
) -> None:
    ...
    while mappings is not None:
        context.get('solutions').append(mappings)
        if len(context.get('solutions')) >= context.get('max_results'):
...
class SageEngine(object):
    ...
    async def execute(
        self, plan: PreemptableIterator, context: Dict[str, Any] = {}
    ) -> ExecutionResults:
        ...
        Throws: Any exception raised during query execution.
        """
        context.setdefault('solutions', [])
        context.setdefault('max_results', float('inf'))
        context.setdefault('timestamp', time())
        query_done = False
        abort_reason = None
        try:
            await executor(plan, context=context)
            query_done = True
