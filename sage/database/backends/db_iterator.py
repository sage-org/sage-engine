...
class DBIterator(ABC):
    """
    A DBIterator follows the iterator protocol and evaluates a triple pattern
    against a RDF dataset.
    Typically, a subclass of this iterator is returned by a call to
    DBConnector#search_pattern.
    """
    ...
    def next(self) -> Optional[Tuple[str, str, str, Optional[datetime], Optional[datetime]]]:
        """
        Return the next RDF triple or raise `StopIteration` if there are no
        more triples to scan.
        """
        pass
...
class EmptyIterator(DBIterator):
    ...
    def next(self) -> None:
        """
        Return the next solution mapping or raise `StopIteration` if there
