...
def explain(query,file,config_file,graph_uri,indentnb,update,parse):
    ...
    root = RootTree()
    source_field = iterator.serialized_name() + '_source'
    pouet=iterator.save()
    getattr(root, source_field).CopyFrom(iterator.save())
    saved_plan=encode_saved_plan(root)
    print("-----------------")
    print("Saving")
    print("-----------------")
    print(root)
